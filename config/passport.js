...
// WeChat Strategy (since we're replacing Facebook and Twitter with WeChat)
const WeChatStrategy = require('passport-wechat').Strategy;

// Note: You'll need to install passport-wechat: npm install passport-wechat
passport.use(new WeChatStrategy({
  ...
  try {
    // Check if user already exists via social account
    let user = await User.findBySocialAccount('wechat', profile.id);

    if (user) {
      return done(null, user);
    }

    // For WeChat, we might not have an email, so we check by display name
    ...
    // Create new user (WeChat might not provide email)
    const userData = {
      name: profile.displayName,
      avatar: profile._json.headimgurl, // WeChat specific
      email: profile.emails ? profile.emails[0]?.value : null
    };
